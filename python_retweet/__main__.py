...
if __name__ == "__main__":
    # API connection
    conf = Configuration()
    authentification = tweepy.OAuthHandler(conf.CONSUMER_KEY, conf.CONSUMER_SECRET)
    authentification.set_access_token(conf.ACCESS_TOKEN, conf.ACCESS_TOKEN_SECRET)
    ...
    while True:
        ...
        random_query = (
            "("
            + conf.SEARCH[randint(0, len(conf.SEARCH) - 1)]
            + ") lang:en OR lang:fr filter:safe"
        )
        print(random_query)
        for tweet in tweepy.Cursor(
            api.search, q=random_query, result_type="recent"
        ).items(TOTAL_RT):
            recovered += 1
